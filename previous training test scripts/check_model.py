...
def check_performance():
    # 1. Load Model
    print(f"🔄 Loading best_model.pth on {DEVICE}...")
    model = smp.Unet(encoder_name='resnet34', classes=len(CLASSES))
    
    try:
        model.load_state_dict(torch.load("best_model.pth"))
    except FileNotFoundError:
        print("❌ Error: best_model.pth not found! Train your model first.")
        return
    ...
    vis_image = cv2.resize(image, (256, 256)) # Save for display
    ...
    # 4. Calculate Simple Accuracy (IoU is harder, this is a quick check)
    ...
    ax[1].imshow(true_mask, cmap='tab10', vmin=0, vmax=9)
    ax[1].set_title("Ground Truth (Target)")
    ax[1].axis('off')

    ax[2].imshow(pred_mask, cmap='tab10', vmin=0, vmax=9)
